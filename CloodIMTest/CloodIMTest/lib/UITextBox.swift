...
extension UIColor {
    ...
    convenience init?(hex:String) {
        ...
        let length = hex.endIndex.distanceTo(hex.startIndex)
        ...
        //        let end = advanceBy(hex.startIndex, result.range.length + result.range.location)        let end = advanceBy(hex.startIndex, result.range.length + result.range.location)
        ...
        let g = CGFloat((number >>  8) & 0xFF) / 255
        let r = CGFloat((number >> 16) & 0xFF) / 255
        let a = end.distanceTo(start) > 6 ? CGFloat((number >> 24) & 0xFF) / 255 : 1
